...
def train_test(model, optimizer, loader, epoch, train, criterion):
...
def main():
    ...
    model_name = "model_best_allcores_fullcores_{0}".format(str(channels))
    ...
    for fold_num, test_slides in enumerate(limited_combinations):
        ...
        for i, core_directory in enumerate(cores_directories):
            ...
            if filter_cores_lowcancer:
                core_file_stats_row = cores_stats_df[(cores_stats_df['cycif.slide'] == core_directory.split('/')[-3]) & (
        ...
        wandb.init(project='pixel_ai',
                   name="resnet_fullcore", resume="never")#, config=config)
        best_f1_test = 0
        best_acc_test = 0
        min_test_loss = float("inf")
        for epoch in range(epochs):
            loss_train, acc_train, f1_train = train_test(resnet50_model, optimizer, train_loader, epoch, train=True,criterion=criterion)
            loss_test, acc_test, f1_test = train_test(resnet50_model, optimizer, test_loader, epoch, train=False,criterion=criterion)
    ...
    wandb.init(project='pixel_ai',
               name="cv_resnet_fullcore", resume="never")#, config=config)
